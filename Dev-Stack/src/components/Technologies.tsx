  ...
  return (
    <div className="container mx-auto">
      ...
      <div className="grid grid-cols-12 gap-6">
            ...
            {data.map((product,index) => {
              return (
                <div key={index} className=" rounded-xl border border-gray-200 bg-white p-4 shadow-sm ">
                  ...
                  {/* Button */}
                  <button onClick={()=> handleAddToCart(product)} className="mt-4 w-full rounded-lg bg-[#080d1c] py-2.5 text-sm font-semibold text-white">
                    Add to Stack
                  </button>
                </div>
              );
        ...
        <div className="col-span-3 bg-yellow-100 h-[500px]"></div>
      </div>
    </div>
  );
};
